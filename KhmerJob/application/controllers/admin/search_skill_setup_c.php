<?php
class Search_skill_setup_c extends CI_Controller
{
	var $pageHeader,$page_redirect;	
	public function __construct()
	{
		parent::__construct();
		$this->pageHeader='Search skill';
		$this->page_redirect="admin/search_skill_setup_c";								
		$this->load->model("search_skill_setup_m");
	}
	...
	{		
		...
		$row=$this->search_skill_setup_m->index();		
		$i=0;
		if($row==TRUE)
		{
	...
	public function add_value()
	{
		if(isset($_POST["btnSubmit"]))
		{
			...
			if($this->validation()==TRUE)
				{									
					$row=$this->search_skill_setup_m->add();															              
	                if($row==TRUE)
	                {	                		                	
	...
	{		
		if($id!="")
		{
			$row=$this->search_skill_setup_m->index($id);			
			if($row==TRUE)
			{																														
				$data['ctrl'] = $this->createCtrl($row);			
	...
	{		
		...
		{						
			if($this->validation()==TRUE)
			{	
				$row=$this->search_skill_setup_m->edit($id);	
				if($row==TRUE)
	            {	                		                	
					redirect("{$this->page_redirect}/");	
	            }																												 																				            	                	                                												
	...
	public function delete($id="")
	{
		if($id!="")
		{
			$row=$this->search_skill_setup_m->delete($id);
