	...
	class product_m_site extends CI_Model
	{
		...
		public function product_page_detail($pro_id)
		{
			if($pro_id!="")
			{
				...
				$query=$this->db->query("SELECT p.p_id, p.p_name, p.price, p.model, p.date_release, p.dimension, p.size, p.p_desc, m.p_id, m.path, c.cat_id, c.cat_name, b.brn_name, s.str_name, pro.pro_type, pd.pro_id, pro.pro_id FROM tbl_promotion AS pro INNER JOIN tbl_promotion_det AS pd ON pro.`pro_id`=pd.`pro_id` RIGHT JOIN tbl_product AS p ON p.`p_id`=pd.`p_id` LEFT JOIN tbl_category AS c ON p.`cat_id`=c.`cat_id` LEFT JOIN tbl_media AS m ON m.`p_id`=p.`p_id` LEFT JOIN tbl_store AS s ON s.`str_id`=p.`str_id` LEFT JOIN tbl_brand AS b ON b.`brn_id`=p.`brn_id` WHERE p.p_id={$pro_id} GROUP BY m.p_id ASC");
				return $query->row();
			}
		}
		#========================== this function product_page_detail =======================
		...
		public function brand($cat_id)
		{
			if ($cat_id!=="") 
			{
				$query=$this->db->query("SELECT * FROM tbl_product AS p INNER JOIN tbl_media AS m ON m.p_id=p.p_id WHERE p.cat_id={$cat_id} GROUP BY m.p_id DESC");
				return $query->result();
			}
		}
